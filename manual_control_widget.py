...
class ManualControlWidget(QWidget):
    ...
    def __init__(self, communicator, parent=None):
        ...
        self.communicator.data_received.connect(self.handle_serial_message)
    ...
    def handle_serial_message(self, line: str):
        line = line.strip()
        if line in ("STATUS:HOMING_COMPLETED", "STATUS:HOMED"):
            # Homing terminato con successo
            self.reset_homing_ui()
            self.is_homed = True
        elif line == "STATUS:STOPPED_BY_USER":
            # Homing interrotto manualmente
            self.reset_homing_ui()
        elif line == "STATUS:TEST_COMPLETED":
            # Se vuoi gestire la fine di un test monotono
            pass
        # ... altri messaggi se servono ...
